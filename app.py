...
def main():
    ...
    cap = cv.VideoCapture(video_path)  # Load video file instead of camera
    ...
    ############### Gud Code ################
    # peace_timestamps = []  # Store timestamps when "peace" sign is detected
    #
    # while True:
    #     ret, image = cap.read()
    #     if not ret:
    #         break  # Video ends
    #
    #     timestamp = cap.get(cv.CAP_PROP_POS_MSEC)  # Get current timestamp in milliseconds
    #     image = cv.cvtColor(image, cv.COLOR_BGR2RGB)
    #     results = hands.process(image)
    #
    #     if results.multi_hand_landmarks is not None:
    ...
    #
    #             if keypoint_classifier_labels[hand_sign_id] == "Peace":
    #                 print(f"Peace sign detected at {timestamp} ms")
    #                 peace_timestamps.append(timestamp)
    #
    #     cv.imshow('Hand Gesture Recognition', cv.cvtColor(image, cv.COLOR_RGB2BGR))
    #
    #     if cv.waitKey(10) & 0xFF == 27:  # Press ESC to exit early
    #         break
    #
    # cap.release()
    # cv.destroyAllWindows()
